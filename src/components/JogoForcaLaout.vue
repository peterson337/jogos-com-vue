<template>
  ...
  <section class="containerJogoForca" >
    ...
    <button type="button" class="btn btn-danger btnCancel" @click="alterarEstado">
      Voltar
    </button>
...
<div class="flexRow">
  <button :id="item"  v-for="item in botoes" :key="item" class="botoes" @click="enviarLetra(item)" 
  >
    {{item}}
  </button>
</div>
     <div class="btnAndInput">
      <input type="text" class="input" maxlength="1" placeholder="Escreva uma letra" @keypress.enter="enviarLetra(input, `input`)"  v-model="input" />
      <button type="button" class="btn btn-info"  @click="enviarLetra(input, `input`)">Enviar letra</button>
    </div> 
  </section>
...
<section class="backgroundModal" v-if="fimDeJogo">
  <div class="modalContent">
    ...
    <div >
     ...
     <h3>Você Perdeu 😞</h3>
      <h4>A palavra correta é: <strong> <i> {{estado}} </i></strong></h4>
      </div>
    ...
    <button type="button" class="btn btn-danger" style="width: 140px; margin: 15px auto" @click="alterarEstado">
      Voltar
    </button>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    alert(`${validarHifen === -1? `A palavra que deve ser adivinhada tem ${palavraFormatada.length} letras e não possuí hífen.` 
 ...
 const enviarLetra = (letra, validationInput) => {
  if(validationInput !== `input`){
   ...
   btnLetra.disabled = true;
      ...
      const ifJogadorGanhou = arrayVersaoFinal.value.map(item => item.letra).join("")
